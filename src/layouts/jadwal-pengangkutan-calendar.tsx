        ...
        <div className="jadwal-cal-card">
            ...
            <div className="jadwal-cal-header">
                <div className="jadwal-cal-header-left">
                    <div className="jbsu-card-title-icon" style={{ background: "#0284c7", borderRadius: 10, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 15, flexShrink: 0 }}>
                        <FaTruckFast />
                    </div>
                    <div>
                                    ...
                                    >
                                        ...
                                        {items.length > 0 && (
                                            <div className="jadwal-cal-chips">
                                                {visible.map(item => {
                                                    const bankLabel = getBankLabel?.(item);
                                                    const cls = `angkut-${item.is_rutin ? "rutin" : "spesial"}`;
                                                    return (
                                                        <div key={item.jadwal_id} className={`jadwal-cal-chip ${cls}`}>
                                                            <span className="jadwal-cal-chip-dot" />
                                                            <span className="jadwal-cal-chip-text">
                                                                {bankLabel ?? formatTime(item.jam_mulai)}
                                                            </span>
                                                        </div>
                                                    );
                                                })}
                    ...
                    <div className="jadwal-cal-detail-list">
                        {selectedItems.map(item => {
                            ...
                            const label     = item.is_rutin
                                ? `Pengangkutan Rutin — ${HARI_DISPLAY[item.hari] || item.hari}`
                                : (item.nama_jadwal_spesial || "Pengangkutan Spesial");
                            return (
                                <div key={item.jadwal_id} className={`jadwal-cal-detail-item ${cls}`}>
                                    ...
                                    {bankLabel && (
                                        <span className="jadwal-cal-detail-bank">{bankLabel}</span>
                                    )}
                                    <span className={`jbsm-detail-badge ${item.is_rutin ? "rutin" : "spesial"}`}>
